...
sd_run <- function(bm) {
  ...
  eval_all <- function(tt, st) {
    env$t <- tt
    for (nm in stock_names) assign(nm, st[[nm]], envir = env)
    for (nm in eval_order) assign(nm, drop_matrix(eval(node_expr[[nm]], env)), envir = env)
    invisible(NULL)
  }
  get_derivs <- function() {
    lapply(stock_names, function(nm) drop_matrix(eval(deriv[[nm]], env)))
  }
  ...
  for (k in seq_len(nsteps + 1L) - 1L) {
    ...
    pending <- if (length(stateful_ids))
      lapply(stateful_in, function(n) get(n, envir = env)) else list()
